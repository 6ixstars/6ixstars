  ...
  return (
    <button
      ...
      style={{
        ...
        color: active ? '#0B0B0C' : 'var(--gray-light)',
        transition: 'all .18s', cursor: 'pointer', borderRadius: 4,
        fontFamily: 'var(--font-sans)', minHeight: 38,
        whiteSpace: 'nowrap',
      }}
    >{children}</button>
  );
...
function FilterGroup({ label, children }) {
  return (
    <div style={{ marginBottom: 22 }}>
      <p style={{ fontSize: '.62rem', letterSpacing: '.28em', textTransform: 'uppercase', color: 'var(--gold)', fontWeight: 800, marginBottom: 10 }}>{label}</p>
      {children}
    </div>
  );
}
...
export default function ShopFilters({
  cat, gender, sort, brand, q,
  typeCats, aromaCats, allBrands,
      ...
      <FilterGroup label="Género">
        <div className="shop-filter-row">
          {(typeCats || []).filter(t => t.id !== 'Todos').map(t => (
            <Pill key={t.id} active={isActive('gender', t.id)} onClick={() => toggleMulti('gender', t.id)}>{t.label}</Pill>
          ))}
        </div>
      </FilterGroup>
    ...
    <>
      ...
      <div className="shop-topbar">
        <div className="shop-search-wrap" style={{ marginBottom: 0, flex: '1 1 auto', minWidth: 0, maxWidth: 'none' }}>
          ...
          <input
            ...
            placeholder="Buscar prendas, marca…"
      ...
      {activeFilters.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 20 }}>
          <span style={{ fontSize: '.68rem', letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--gold)', fontWeight: 800, marginRight: 4 }}>
            Activos
          </span>
          {activeFilters.map(f => (
            <button
