...
import { ShoppingBag, Search, Menu, X, Heart, ChevronDown, ChevronRight, ArrowRight, FileText, Truck, RotateCcw, HelpCircle, Mail } from 'lucide-react';
import { useCartStore, useCartCount } from '@/lib/store/cartStore';
import { useWishlistStore } from '@/lib/store/wishlistStore';
import { collections, productTypes, getImagePath } from '@/lib/products-constants';
...
function buildNavItems(allBrands) {
  return [
    { label: 'Inicio', to: '/' },
    {
      ...
      categories: [
        ...
        },
        {
          id: 'genero',
          label: 'Género',
          items: productTypes.map(g => ({ label: g.name, desc: g.description, to: `/tienda?gender=${g.id}` })),
        },
        {
          id: 'marcas',
          label: 'Marcas',
    ...
    { label: 'Drops', to: '/tienda?sort=nuevo' },
...
function NavbarInner({ products = [] }) {
  ...
  const [openSubcat, setOpenSubcat] = useState('categorias');
  ...
  // Resultados en vivo (productos, marcas, categorías, páginas)
  const searchResults = useMemo(() => {
    ...
    const categoryMatches = collections.filter(c =>
    ...
    return {
      ...
      empty: productMatches.length === 0 && brandMatches.length === 0 &&
             categoryMatches.length === 0 && pageMatches.length === 0,
    };
  }, [searchQuery]);
  ...
  const isItemActive = (item) => {
    ...
    if (item.to.includes('sort=nuevo')) return searchParams?.get('sort') === 'nuevo';
          ...
          <div className="container">
            <div className="sb-search-inputwrap">
              ...
              <input
                ...
                placeholder="Buscar prendas, categorías o páginas..."
                autoFocus
                value={searchQuery}
            ...
            <div className="sb-drawer-body">
              ...
              {NAV_ITEMS.filter(i => i.isMegaMenu).map(item => (
                <div key={item.label} className="sb-drawer-section">
                  <p className="sb-drawer-section-title">Explorar</p>
                  {item.categories.map(cat => {
...
function NavStyles() {
  return (
    <style>{`
      ...
      .sb-nav-link-wrap { position: relative; display: flex; align-items: center; }
      .sb-nav-link {
      ...
      .sb-megamenu-item:hover .sb-dropdown-dot {
        box-shadow: 0 0 0 2px var(--gold), 0 0 14px currentColor;
        transform: scale(1.1);
      }

      .sb-nav-actions { display: flex; gap: 4px; align-items: center; justify-self: end; }
      .sb-nav-icon-btn {
        position: relative;
      ...
      .sb-megamenu-item:focus-visible { outline: 2px solid var(--gold); outline-offset: -2px; }
      .sb-nav-badge {
        position: absolute;
        top: 4px; right: 4px;
        background: var(--gold);
      ...
      .sb-search-input:focus {
        ...
        box-shadow: 0 0 0 4px rgba(255,46,126,.18);
      }
      .sb-search-clear {
        position: absolute;
        right: 10px;
      ...
      .sb-search-product-info .price { font-size: .76rem; color: var(--gold); font-weight: 700; margin-top: 1px; }
      .sb-search-viewall {
      ...
      .sb-search-page .desc { display: block; font-size: .72rem; color: var(--gray); margin-top: 1px; }
      ...
      .sb-drawer-body { flex: 1; overflow-y: auto; padding: 12px 0 24px; scrollbar-width: none; }
      ...
      .sb-drawer-main-nav { padding: 0 8px; margin-bottom: 8px; }
      .sb-drawer-main-link {
      ...
      .sb-drawer-section { padding: 0 8px; border-top: 1px solid var(--dark-4); padding-top: 12px; margin-top: 4px; }
      ...
      .sb-drawer-cat-btn:hover, .sb-drawer-cat-btn[aria-expanded="true"] { background: rgba(255,46,126,.1); color: var(--gold); }
      .sb-drawer-chevron { color: var(--gray); transition: transform .25s; flex-shrink: 0; }
      .sb-drawer-chevron.open { transform: rotate(180deg); color: var(--gold); }
      ...
      .sb-drawer-cat-link:hover, .sb-drawer-cat-link:active { background: rgba(255,46,126,.1); color: var(--gold); }
      .sb-drawer-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; box-shadow: 0 0 0 2px rgba(255,255,255,.15); }
      ...
      .sb-drawer-chip:hover, .sb-drawer-chip:active { background: rgba(255,46,126,.18); color: var(--gold); border-color: var(--gold); }
      ...
      /* ===== MEGA MENU (desktop, dos paneles oscuros) ===== */
      .sb-megamenu-wrap { left: 0; transform: none; animation: sb-drop-fade .22s cubic-bezier(.22,.61,.36,1); }
      .sb-megamenu {
        display: flex;
        border-radius: 10px;
        overflow: hidden;
      ...
      .sb-megamenu-row:hover { background: rgba(255,46,126,.1); color: var(--white); }
      .sb-megamenu-row.active { background: rgba(255,46,126,.18); color: var(--gold); }
      .sb-megamenu-row.active svg { color: var(--gold); }
      ...
      .sb-megamenu-ver-todo:hover { color: var(--gold); }
      .sb-megamenu-right {
        flex: 1;
        background: #1A1A1D;
        padding: 14px;
      ...
      .sb-megamenu-item:hover { background: rgba(255,46,126,.12); transform: translateX(2px); }
      ...
      .sb-megamenu-item:hover .sb-megamenu-item-label { color: var(--gold); }
      .sb-megamenu-item-desc { font-size: .68rem; color: var(--gray); margin: 2px 0 0; line-height: 1.3; }

      @media (max-width: 1024px) { .sb-nav-links { gap: 20px; } }
      @media (max-width: 900px) {
        .sb-nav-links { display: none; }
        .sb-mobile-toggle-left { display: inline-flex; }
        .sb-nav-inner { grid-template-columns: 1fr auto 1fr; gap: 12px; padding: 8px 0; }
      }
    `}</style>
  );
}
