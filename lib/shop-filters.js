...
export function filterAndSort(products, {
  ...
  const genders = vals(gender).map(g => g.toLowerCase());
  ...
  // Comparación case-insensitive: los links del footer mandan gender=hombre/mujer
  // en minúsculas, pero el valor guardado en products.gender es "Hombre"/"Mujer"
  // (como lo escribe el form del admin) — sin esto, esos links no filtraban nada.
  if (genders.length)  list = list.filter(p => genders.includes((p.gender || '').toLowerCase()));
