...
async function _getAllProductsFromDb() {
  if (!supabaseAdmin) {
    // Sin Supabase (desarrollo local): usar productos de ejemplo para
    // poder previsualizar home/tienda con cards reales. En producción
    // supabaseAdmin está configurado y esta rama nunca se ejecuta.
    const { SAMPLE_PRODUCTS } = await import('./sample-products.js');
    console.warn(`[products-db] supabaseAdmin no disponible — usando ${SAMPLE_PRODUCTS.length} productos de ejemplo (solo local)`);
