...
export async function createPaymentLink({
  ...
  const res = await fetch(BOLD_LINKS_API_URL, {
    ...
    body: JSON.stringify({
      ...
      // Sin expiration_date: Bold aplica su propio default. El valor que
      // mandábamos antes (epoch ms) lo rechazaba con "expiration date must
      // be >= current date" — no está confirmado si Bold espera otra unidad
      // (segundos/nanosegundos) así que mejor omitirlo que adivinar.
  ...
  if (!res.ok || !json?.payload?.url) {
    // Shape real de error de Bold: { payload: {}, errors: [{ errors: "<mensaje>", code }] }
    const msg = json?.errors?.[0]?.errors || json?.errors?.[0]?.message || json?.message
      || `Bold API error: ${res.status} — ${rawText.slice(0, 500)}`;
    throw new Error(msg);
  }
