...
export async function createPaymentLink({
  ...
  if (!res.ok || !json?.payload?.url) {
    // TODO(diagnóstico temporal): incluir el body crudo hasta confirmar el
    // shape real de los errores de Bold; después volver a un mensaje corto.
    const msg = json?.errors?.[0]?.message || json?.message || `Bold API error: ${res.status} — ${rawText.slice(0, 500)}`;
    throw new Error(msg);
  }
