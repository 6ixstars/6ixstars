...
const FROM_EMAIL = process.env.FROM_EMAIL || '6ixstars <onboarding@resend.dev>';
...
export async function sendOrderEmail(order, items = []) {
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,sans-serif;max-width:600px;margin:0 auto;color:#1f2937">
      <div style="background:linear-gradient(135deg,#AF1F3A,#A8895A);padding:24px;text-align:center;color:#fff">
      ...
      <div style="padding:24px;background:#fff">
        ...
        <a href="https://6ixstars.com.co/admin/orders/${order.id}"
      ...
      <div style="padding:16px;text-align:center;color:#9ca3af;font-size:12px">
        6ixstars · Notificación automática
      </div>
    </div>
  `;
...
export async function sendOrderWhatsApp(order, items = []) {
  ...
  const message =
    ...
    `🔗 https://6ixstars.com.co/admin/orders/${order.id}`;
...
<head>
  ...
  <title>Pedido confirmado · 6ixstars</title>
</head>
<body style="margin:0;padding:0;background:#F5EFE3;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif">
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="max-width:600px;background:#FFFFFF;border-radius:14px;overflow:hidden;box-shadow:0 4px 24px rgba(31,26,20,.06)">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#1F1A14 0%,#2A2218 100%);padding:48px 32px;text-align:center">
              <div style="display:inline-block;padding:14px 22px;border:1px solid rgba(175,31,58,.4);border-radius:4px">
                <div style="font-family:Georgia,'Times New Roman',serif;font-size:11px;letter-spacing:.45em;color:#AF1F3A;text-transform:uppercase">6ixstars</div>
                <div style="font-family:Georgia,'Times New Roman',serif;font-size:9px;letter-spacing:.32em;color:rgba(238,177,195,.6);text-transform:uppercase;margin-top:3px">Ropa urbana</div>
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:56px 48px 32px;text-align:center">
              <div style="font-size:11px;letter-spacing:.32em;text-transform:uppercase;color:#AF1F3A;font-weight:600;margin-bottom:18px">
                Pedido confirmado
              </div>
              ...
              <p style="font-size:15px;color:#4A3F33;line-height:1.65;margin:0;max-width:420px;display:inline-block">
                Hemos recibido tu pedido y tu pago fue procesado correctamente. Estamos preparando tu pedido con el cuidado que merece.
              </p>
            </td>
          </tr>
            ...
            <td style="padding:8px 48px 0">
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                ...
                <tr>
                  ...
                  <td style="padding:14px 0 0;font-family:Georgia,'Times New Roman',serif;font-size:22px;color:#AF1F3A;text-align:right;font-weight:600">${formatCOP(total)}</td>
                </tr>
              </table>
            </td>
          </tr>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                <tr>
                  <td style="padding-bottom:18px;vertical-align:top;width:36px">
                    <div style="width:28px;height:28px;border-radius:50%;background:#AF1F3A;text-align:center;line-height:28px;color:#fff;font-weight:600;font-size:13px">1</div>
                  </td>
                  <td style="padding-bottom:18px;padding-left:16px;vertical-align:top">
                    ...
                    <div style="font-size:13px;color:#7A6E5E;line-height:1.5">Validamos stock y empacamos tu pedido con material protector.</div>
                  </td>
                </tr>
                <tr>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#FAF6EE;border-radius:10px;border:1px solid #EFEAE0">
                <tr>
                  <td style="padding:24px;text-align:center">
                    ...
                    <a href="https://wa.me/573143776839?text=${encodeURIComponent('Hola! Tengo una consulta sobre mi pedido ' + order.reference)}"
                    ...
                    <a href="mailto:ventas@6ixstars.com.co?subject=${encodeURIComponent('Consulta pedido ' + order.reference)}"
          ...
          <tr>
            <td style="background:#1F1A14;padding:36px 48px;text-align:center">
              <div style="font-family:Georgia,'Times New Roman',serif;font-size:12px;letter-spacing:.42em;color:#AF1F3A;text-transform:uppercase;margin-bottom:14px">6ixstars</div>
              <div style="font-size:11px;color:rgba(250,246,238,.55);line-height:1.7;letter-spacing:.04em">
                Ropa urbana · Colombia<br/>
                <a href="https://6ixstars.com.co" style="color:rgba(250,246,238,.7);text-decoration:none">6ixstars.com.co</a>
              </div>
              <div style="margin-top:20px;padding-top:20px;border-top:1px solid rgba(250,246,238,.08);font-size:10px;color:rgba(250,246,238,.35);line-height:1.6">
                Recibiste este correo porque realizaste un pedido en 6ixstars.<br/>
  ...
  try {
    const result = await resend.emails.send({
      ...
      subject: `Tu pedido está confirmado · 6ixstars`,
      html,
    });
    return { sent: true, id: result.data?.id };
  } catch (err) {
...
export async function sendAbandonedCartEmail(order, items = []) {
  ...
  const checkoutUrl = `https://6ixstars.com.co/checkout`;
...
<body style="margin:0;padding:0;background:#F5EFE3;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif">
  ...
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent">
    Dejaste ${itemCount} ${itemCount === 1 ? 'producto' : 'productos'} en tu carrito por ${formatCOP(total)}. Te los guardamos.
  </div>
    ...
    <tr><td align="center">
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="max-width:600px;background:#FFFFFF;border-radius:14px;overflow:hidden;box-shadow:0 4px 24px rgba(31,26,20,.06)">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#1F1A14 0%,#2A2218 100%);padding:48px 32px;text-align:center">
            <div style="display:inline-block;padding:14px 22px;border:1px solid rgba(175,31,58,.4);border-radius:4px">
              <div style="font-family:Georgia,serif;font-size:11px;letter-spacing:.45em;color:#AF1F3A;text-transform:uppercase">6ixstars</div>
              <div style="font-family:Georgia,serif;font-size:9px;letter-spacing:.32em;color:rgba(238,177,195,.6);text-transform:uppercase;margin-top:3px">Ropa Urbana</div>
            </div>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:56px 48px 28px;text-align:center">
            ...
            <p style="font-size:15px;color:#4A3F33;line-height:1.65;margin:0;max-width:440px;display:inline-block">
              Dejaste ${itemCount} ${itemCount === 1 ? 'producto preparado' : 'productos preparados'} para llevar. Los guardamos por si quieres completar tu compra cuando estés listo.
            </p>
          </td>
        </tr>
          ...
          <td style="padding:24px 48px 0">
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
              <tr>
                ...
                <td style="padding:14px 0 0;font-family:Georgia,serif;font-size:22px;color:#AF1F3A;text-align:right;font-weight:600">${formatCOP(total)}</td>
              </tr>
            </table>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:40px 48px;text-align:center">
            ...
            <p style="font-size:13px;color:#7A6E5E;line-height:1.6;margin:24px 0 0">
              Envío gratis a toda Colombia · Calidad garantizada
            </p>
          </td>
        </tr>
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#FAF6EE;border-radius:10px;border:1px solid #EFEAE0">
              <tr>
                <td style="padding:20px 24px;text-align:center">
                  ...
                  <a href="https://wa.me/573143776839?text=${encodeURIComponent('Hola! Tengo dudas sobre mi carrito en 6ixstars')}"
                     style="display:inline-block;margin-top:10px;color:#AF1F3A;text-decoration:underline;font-size:13px;font-weight:600">
        ...
        <tr>
          <td style="background:#1F1A14;padding:36px 48px;text-align:center">
            <div style="font-family:Georgia,serif;font-size:12px;letter-spacing:.42em;color:#AF1F3A;text-transform:uppercase;margin-bottom:14px">6ixstars</div>
            <div style="font-size:11px;color:rgba(250,246,238,.55);line-height:1.7;letter-spacing:.04em">
              Ropa urbana · Colombia<br/>
              <a href="https://6ixstars.com.co" style="color:rgba(250,246,238,.7);text-decoration:none">6ixstars.com.co</a>
            </div>
  ...
  try {
    const result = await resend.emails.send({
      from: FROM_EMAIL,
      to: order.customer_email,
      subject: `¿Olvidaste algo${firstName ? ', ' + firstName : ''}? · 6ixstars`,
      html,
    });
    return { sent: true, id: result.data?.id };
  } catch (err) {
...
// === Envío unificado ===
// Llamado desde el webhook de Bold cuando una orden es aprobada.
